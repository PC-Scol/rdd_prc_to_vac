...
declare 	
	...
	cur_element_fils cursor for select distinct code_apprenant ||'-' || code_objet_formation ||'-' || code_periode ||'-' || code_filtre_formation id
										 ...
										 where not exists (select 1 from apprenant_chc appchc where appchc.code_apprenant = appcoc.code_apprenant and (appcoc.code_objet_formation = appchc.code_objet_formation) and appchc.code_filtre_formation = appcoc.code_filtre_formation and appcoc.code_periode = appcoc.code_periode)
